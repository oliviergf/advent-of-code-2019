...
def intComputer_part2(commands, index, onlyInput, outputs):
    ...
    if op_code is not '9':
        first = commands[commands[index+1]
                         ] if instruction[2] == '0' else commands[index+1]
        second = commands[commands[index+2]
                          ] if instruction[3] == '0' else commands[index+2]
        if op_code == '5':
            pos_to_jump_to = second if first != 0 else index + 3
            intComputer_part2(commands, pos_to_jump_to, onlyInput, outputs)
        elif op_code == '6':
            pos_to_jump_to = second if first == 0 else index + 3
            intComputer_part2(commands, pos_to_jump_to, onlyInput, outputs)
        elif op_code == '3':
            ...
            intComputer_part2(commands, index + 2, onlyInput, outputs)
        elif op_code == '4':
            if instruction[2] == "1":
